#include <math.h>
// TODO: Get rid of c math header
#define Abs(a) ((a) > 0 ? (a) : -(a))
#define Mod(a, m) (((a) % (m)) >= 0 ? ((a) % (m)) : (((a) % (m)) + (m)))

static float DegToRad(float degrees) { return degrees * (PI32/180.0f); }
static float RadToDeg(float radians) { return radians * (180/PI32); }
...
static Vec4 V4FromV3(Vec3 xyz, float w) { return Vec4 { xyz.x, xyz.y, xyz.z, w }; }
...
static Mat4 M4Perspective(float fov, float aspect_ratio, float Near, float Far) {
	Mat4 result ={};
	float cot = 1.0f / tanf(fov * (PI32/360.0f));
